...
#pragma warning restore
		...
		[SerializeField] private float m_btMinDistance = 50.0f;
		private GameObject UI_Scope = null;
		private PlayerWeaponController m_playerWeaponController = null;
		...
		protected override void Awake()
		{
			...
			UI_Scope = GameObject.Find("Canvas/GameUI/FX").gameObject;
			m_playerWeaponController = GameObject.Find("Weapon").gameObject.GetComponent<PlayerWeaponController>();
			Debug.Log("テスト："+UI_Scope.activeSelf);
		}
		...
		private IEnumerator FireBallisticsBulletRoutine(BulletProperties properties, Transform bulletTimeFireTransform, Transform bulletTimeBullet, bool checkForBulletTime)
		{
			...
			while (remainingTime > 0.0f)
			{
				...
				if (!skipNormalRay)
				{
					if (Physics.Raycast(new Ray(rOrigin, rDirection.normalized), out hit, rDirection.magnitude, m_rayMask))
					{
						...
						if (checkForBulletTime && !foundTarget)
						{
							...
							if (btt && btt.IsActive && dist > m_btMinDistance)
							{
								// Set target as found.
								foundTarget = btt;
								if(UI_Scope.activeSelf) UI_Scope.SetActive(false);
								m_playerWeaponController.scopeMode = false;
							}
						}
					...
					{
						float timer = 0.0f;
						float targetWait = stepTime - accumulatedWaitTime;

						Vector3 targetPosition = finalRayPosition;

						while (timer < targetWait)
						{
						...
						accumulatedWaitTime = (timer - targetWait);
					}
				}
				else
					skipNormalRay = false;
